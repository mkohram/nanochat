...
Examples:
    ...
    torchrun --nproc_per_node=8 -m scripts.local_base_eval --hf-path openai-community/gpt2

    # Evaluate a nanochat model (e.g. d24) using 8 GPUs
    torchrun --nproc_per_node=8 -m scripts.local_base_eval --model-tag d24 --device-batch-size=16

    # Quick/approximate evaluation using a single GPU
    python -m scripts.local_base_eval --model-tag d24 --device-batch-size=16 --max-per-task=100 --split-tokens=524288
"""
import os
import csv
import time
...
def evaluate_core(model, tokenizer, device, max_per_task: int = -1):
    """
    ...
    for task in tasks:
        ...
        context_failures[label] = int(task_meta.get('context_failures', 0))
        random_baseline = random_baselines[label]
        centered_result = (accuracy - 0.01 * random_baseline) / (1.0 - 0.01 * random_baseline)
        centered_results[label] = centered_result
        elapsed = time.time() - start_time
        print0(f"accuracy: {accuracy:.4f} | centered: {centered_result:.4f} | ctx_failures: {context_failures[label]} | time: {elapsed:.2f}s")
...
def main():
    ...
    if is_hf_model:
        ...
        # Keep eval dtype consistent on CUDA to avoid bf16/float mismatches in some CORE tasks.
        if device_type == "cuda":
